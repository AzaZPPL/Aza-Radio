...
public Plugin myinfo = 
{
	...
	version = "1.0", 
	url = "https://github.com/AzaZPPL/Aza-Radio"
};
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_radiostop", StopRadio);
...
{
	PrintToChat(client, "%s", " \x03[\x02Aza-Radio\x03]\x01 Welcome! This server is using Aza-Radio!");
	PrintToChat(client, "%s", " \x03[\x02Aza-Radio\x03]\x01 Type in !radio for the radio menu!");
...
public void SetupMenu()
{
	...
	stationsmenu.SetTitle("Aza-Radio Options");
	...
	headmenu.SetTitle("Aza-Radio Options");
...
public Action Change_Volume(int client, int args)
{
	...
	FormatEx(url, sizeof(url), "https://junkfoodmountain.com/plugins/radio/%s/%i", h_client[client][Radio_Url], vol);
	
	StreamPanel("Aza-Radio", url, client);
}
...
public int VolumeMenuHandler(Menu menu, MenuAction action, int client, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		FormatEx(url, sizeof(url), "https://junkfoodmountain.com/plugins/radio/%s/%i", h_client[client][Radio_Url], h_client[client][Radio_Vol]);
...
public int StationsMenuHandler(Menu menu, MenuAction action, int client, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		PrintToChatAll(" \x03[\x02Aza-Radio\x03]\x01 %s Started listening to %s", h_client[client][Name], h_client[client][Radio_Name]);
...
public void LoadWebshortcuts()
{
	...
	if (f == INVALID_HANDLE)
	{
		LogError("[Aza-Radio] Could not open file: %s", buffer);
		return;
	}
...
public Action StopRadio(int client, int args)
{
	if (client > 0 && client <= MaxClients && IsClientInGame(client))
	{
		StreamPanel("Aza-Radio", "Thanks for listening", client);
	}
} 